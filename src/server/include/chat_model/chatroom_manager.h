...
typedef shared_ptr<RoomSpace::ChatRoom> ChatRoom_shrd_ptr;

/* This struct holds chatrooms sorted and binded by their owner unique_token */
typedef unordered_map<std::string, vector<ChatRoom_shrd_ptr>> ChatRoom_Map_Type;

/* This struct holds chatrooms binded to their own chat ids in one map */
typedef unordered_map<std::string, ChatRoom_shrd_ptr> ChatRoom_Map_Glob_Type;
class ChatRoomManager {
 public:
  ...
  vector<ChatRoom_shrd_ptr>& push_new_room(Customer::CustomerModel*,
                                           RoomSpace::ChatRoom*);
  vector<ChatRoom_shrd_ptr>& remove_room_from(Customer::CustomerModel*,
